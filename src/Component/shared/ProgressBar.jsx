...
export default function ProgressBar() {
    const { data } = useFetch(`${baseURL}/api/Profile/calculate-course-percentage`);
    const percentage = data?.hoursePersentage || 0;
     return (
        <div className="sticky top-0 xl:top-[13.7rem] z-50 xl:z-10 max-w-sm sm:max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-6xl w-full h-8 bg-gray-300 rounded-full overflow-hidden mx-auto mb-24">
            ...
            <span className={`absolute top-1/2  transform  -translate-y-1/2 text-white text-sm sm:text-lg font-semibold`} style={{ left: `calc(${percentage / 2}% - 1rem)` }}>
                {percentage}%
            </span>
        </div>
    );
